...
router.get("/news-da-xoa", async (req, res) => {
    try {
        const news = await TinTucModel.findAll({
        ...
        const news_with_category = news.map(item => {
            return {
                ...item.toJSON(),
                category: news_category.get(item.id_loai)
            }
...
router.post("/dangnhap", async (req, res) => {
    try {
        ...
        if (user.vai_tro != 1) return res.status(401).json({ message: "Bạn không phải admin" });
        if (user.khoa == 1) return res.status(401).json({ message: "Tài khoản đã bị khóa" });

        if (!user.mat_khau || typeof user.mat_khau !== "string") {
            return res.status(500).json({ message: "Dữ liệu mật khẩu không hợp lệ trong hệ thống" });
        }
        ...
        if (!privateKey) {
            return res.status(500).json({ message: "Không tìm thấy khóa bí mật" });
        }
        ...
        sendMail(
            email,
            "Xác nhận đăng nhập",
            `<h3>Chào ${user.ho_ten}, bạn đã đăng nhập thành công.</h3>`
        );
