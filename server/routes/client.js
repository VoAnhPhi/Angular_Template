...
router.get("/loai/:id", async (req, res) => {
    try {
        const loai = await LoaiModel.findOne({
            where: {
                id: req.params.id,
                an_hien: 1
            },
            attributes: ['id', 'ten_loai', 'slug']
        })
        if (!loai) {
        ...
        res.status(500).json({ error: "Lỗi server", error: err.message });
    }
})
...
router.post('/luugiohang', async (req, res) => {
    ...
    await DonHangChiTietModel.create({
        ...
        .then(function (item) {
            res.json({ status: 200, "thong_bao": "Đã lưu giỏ hàng", "sp": item });
        })
        .catch(function (err) {
            res.json({ status: 500, "thong_bao": "Lỗi lưu giỏ hàng ", err })
        });
});
